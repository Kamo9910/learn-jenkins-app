pipeline {
    ...
    environment {
        NETLIFY_SITE_ID = 'a43a208e-c546-4d73-8c7b-1294cf417545'
    ...
    stages {
        stage('AWS') {
            ...
            steps{
                withCredentials([usernamePassword(credentialsId: 'my-aws-credentials', passwordVariable: 'AWS_SECRETE_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
                sh'''
                    aws --version
                    aws s3 ls
                '''
       ...
       stage('Build') {
        ...
        stage ('Deploy Prod') {
            ...
            steps {
                ...
                node --version
                netlify --version
                echo "Deploying to production. Sit ID:  $NETLIFY_SITE_ID "
                netlify status
